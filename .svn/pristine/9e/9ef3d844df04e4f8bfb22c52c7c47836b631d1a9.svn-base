...
{{#class 'ETFramework.app.Controller'}}
	...
	toTrueView: function(item, index, target, record, e, eOpts) {
		...
		if(record.get('moreapp_id') == '124-update-678') {//版本更新
			var updateController = this.getApplication().getController('{{controller "UpdateController"}}'); 
			...
			var mainController = this.getApplication().getController('{{controller "SignUpController"}}'); 
			mainController.toUpdate();
		}else if(record.get('moreapp_id') == '124-freecheck-678'){
			var page = 'freeCheck';
			var store = 'freeStore';
			var path = '{{view "free.FreeCheck"}}';
