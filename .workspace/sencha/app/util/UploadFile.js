Ext.define('VisLeader.util.UploadFile', {	singleton: true,
	...
	*/
	...
	uploadPicture: function(path,store,component,otherBusiness) {
		...
		var webUrl = bcpConfig.loginURL; //localStorage.mybase_url;
	...
	uploadSuccessCallBack: function(message,path,store,component){
		...
		var status = (JSON.parse(json.response)).status;
		var file = (JSON.parse(json.response)).retmsg;
		console.log(state);
		if(state == 200 || state == "200") {
			if(file != "false" && status != false){
				console.log('图片上传成功');
	...
	*/
	...
	clearData: function(store,component,isOne) { 
